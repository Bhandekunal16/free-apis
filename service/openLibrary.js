...
const config = require("../jsons/openLibrary.json");
...
class OpenLibrary {
  ...
  constructor() {
    this.#baseUrl = route.openLibrary;
  ...
  async #apiCall(options) {
    ...
    if (endpoint.includes("{subject}")) {
      ...
      endpoint = endpoint.replace("{subject}", encodeURIComponent(value));
    }

    if (endpoint.includes("{isbn}")) {
      if (!value) {
        return {
          success: false,
          statusCode: 400,
      ...
      endpoint = endpoint.replace("{isbn}", encodeURIComponent(value));
    }
    ...
    try {
      ...
      return {
        success: false,
        statusCode: 502,
        message: "Failed to connect to Open Library API",
        error: error.message,
        url: requestUrl,
      };
    } finally {
