...
const config = require("../jsons/openFoodFacts.json");
...
class OpenFoodFacts {
  ...
  constructor() {
    this.#baseUrl = route.openFoodFacts;
  ...
  async #apiCall(options) {
    ...
    // Category is a convenience endpoint.
    // Open Food Facts uses /search with categories_tags_en.
    if (type === "category") {
      ...
      searchParams.set("categories_tags_en", String(value));
    }
    ...
    if (type === "brand") {
      if (value === undefined || value === null || value === "") {
        return {
          success: false,
          statusCode: 400,
          message: "value is required for type 'brand'",
        };
      }

      endpoint = endpoints.products;

      searchParams.set("brands_tags_en", String(value));
    }
    ...
    if (placeholders) {
      ...
      endpoint = endpoint.replace(/\{[^}]+\}/g, encodeURIComponent(value));
    }
    ...
    try {
      ...
      return {
        success: false,
        statusCode: 502,
        message: "Failed to connect to Open Food Facts API",
        error: error.message,
        url: finalUrl,
      };
    } finally {
