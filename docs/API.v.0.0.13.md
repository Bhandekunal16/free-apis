...
The Random User API provides randomly generated user profiles with support
for filtering by gender, nationality, selected fields, exclusions, seeds,
...
| --------- | -------- | ----------- |
| `type` | No | Operation type; defaults to `random` |
| `results` | No | Number of users to return |
| `gender` | No | Filter by `male` or `female` |
| `nat` | No | Comma-separated nationality codes |
| `seed` | No | Seed used to generate consistent results |
| `page` | No | Page number when using a seed |
| `inc` | No | Comma-separated fields to include |
| `exc` | No | Comma-separated fields to exclude |
| `format` | No | Response format |
| `noinfo` | No | Exclude the `info` object |

All query parameters other than the local `type` parameter are forwarded
to the Random User API.

### Supported Types

The supported operation type is `random`.

| Type | Upstream path | Description |
| ---- | ------------- | ----------- |
| `random` | `/` | Generate random user profiles |

### Examples

```http
GET /random-user
GET /random-user?results=10
GET /random-user?gender=male
GET /random-user?gender=female
GET /random-user?nat=in
GET /random-user?results=10&nat=in
GET /random-user?results=10&inc=name,email,picture
GET /random-user?results=10&exc=login
GET /random-user?results=10&seed=foobar
GET /random-user?results=10&page=2&seed=foobar
```

The default type is `random`. Query parameters are forwarded to the upstream
API. Invalid operation types return `400`; upstream errors and timeouts are
propagated as documented below.
...
The Bored API provides random activities and supports filtering activities
by type and number of participants, as well as retrieving an activity by key.
...
| Parameter | Required | Description |
| --------- | -------- | ----------- |
| `operation` | No | Operation type; defaults to `random` |
| `value` | Conditional | Activity key for `activity` |
| `type` | Conditional | Activity type filter for `filter` |
| `participants` | Conditional | Number of participants for `filter` |

### Supported Operations

Supported operation types are `random`, `filter`, and `activity`.

| Operation | Upstream path | Description |
| --------- | ------------- | ----------- |
| `random` | `/random` | Get a random activity |
| `filter` | `/filter` | Filter activities |
| `activity` | `/activity/{key}` | Get an activity by key |
...
GET /bored?operation=filter&type=social&participants=2
GET /bored?operation=activity&value=3943506
```

The default operation is `random`. The `filter` operation forwards supported
filter parameters such as `type` and `participants`. The `activity` operation
requires an activity key. Invalid operation types or missing required values
return `400`.

The local operation parameter is named `operation` so it does not conflict
with the upstream API's `type` filter parameter.

---

# 25. Deck of Cards API

The Deck of Cards API provides operations for creating decks, shuffling cards,
drawing cards, and managing named piles.
...
| Parameter | Required | Description |
| --------- | -------- | ----------- |
| `operation` | No | Operation type; defaults to `newShuffle` |
| `deckId` | Conditional | Existing deck ID for deck operations |
| `pileName` | Conditional | Pile name for pile operations |
| `count` | Conditional | Number of cards to draw |
| `deckCount` | Conditional | Number of decks to create |
| `jokersEnabled` | Conditional | Include two Jokers in a new deck |
| `cards` | Conditional | Comma-separated card codes |
| `remaining` | Conditional | Shuffle only remaining cards when `true` |

### Supported Operations

Supported operation types include `new`, `newShuffle`, `draw`, `shuffle`,
`return`, `pileAdd`, `pileShuffle`, `pileList`, `pileDraw`, and `pileReturn`.

| Operation | Upstream path | Description |
| --------- | ------------- | ----------- |
| `new` | `/new/` | Create a new unshuffled deck |
| `newShuffle` | `/new/shuffle/` | Create and shuffle a new deck |
| `draw` | `/{deckId}/draw/` | Draw cards from an existing deck |
| `shuffle` | `/{deckId}/shuffle/` | Reshuffle an existing deck |
| `return` | `/{deckId}/return/` | Return cards to the main deck |
| `pileAdd` | `/{deckId}/pile/{pileName}/add/` | Add cards to a pile |
| `pileShuffle` | `/{deckId}/pile/{pileName}/shuffle/` | Shuffle a pile |
| `pileList` | `/{deckId}/pile/{pileName}/list/` | List cards in a pile |
| `pileDraw` | `/{deckId}/pile/{pileName}/draw/` | Draw cards from a pile |
| `pileReturn` | `/{deckId}/pile/{pileName}/return/` | Return cards from a pile to the deck |
...
GET /deck-of-cards?operation=new&jokersEnabled=true
...
GET /deck-of-cards?operation=return&deckId=3p40paa87x90&cards=AS,2S
GET /deck-of-cards?operation=pileAdd&deckId=3p40paa87x90&pileName=discard&cards=AS,2S
GET /deck-of-cards?operation=pileList&deckId=3p40paa87x90&pileName=discard
GET /deck-of-cards?operation=pileDraw&deckId=3p40paa87x90&pileName=discard&count=2
```

The default operation is `newShuffle`. Operations that work with an existing
deck require `deckId`. Pile operations require both `deckId` and `pileName`.
Card lists use card codes such as `AS`, `2S`, and `KH`.

The local operation parameter is named `operation` so it can be used alongside
the upstream API parameters such as `cards`, `count`, and `remaining`.

---

# 26. Chess.com Public API

The Chess.com Public API provides access to public player, club, country,
leaderboard, titled-player, streamer, puzzle, and game data.
...
| Parameter | Required | Description |
| --------- | -------- | ----------- |
| `type` | No | Operation type; defaults to `dailyPuzzle` |
| `username` | Conditional | Chess.com username for player operations |
| `club` | Conditional | Chess.com club URL/name for club operations |
| `country` | Conditional | Two-letter country code for country operations |
| `year` | Conditional | Four-digit year for `playerGames` |
| `month` | Conditional | Two-digit month for `playerGames` |
| `title` | Conditional | Chess title for `titled`, such as `GM` |
| `value` | Conditional | Resource ID for `puzzle` or `game` |
| `query` | No | Additional upstream query parameters |

### Supported Types

Supported operation types are `dailyPuzzle`, `randomPuzzle`, `puzzle`,
`player`, `playerStats`, `playerArchives`, `playerGames`, `club`,
`clubMembers`, `clubMatches`, `country`, `countryPlayers`, `countryClubs`,
`titled`, `leaderboards`, and `streamers`.

| Type | Upstream path | Description |
| ---- | ------------- | ----------- |
| `dailyPuzzle` | `/puzzle` | Get the daily puzzle |
| `randomPuzzle` | `/puzzle/random` | Get a random puzzle |
| `puzzle` | `/puzzle/{id}` | Get a puzzle by ID |
| `player` | `/player/{username}` | Get player profile |
| `playerStats` | `/player/{username}/stats` | Get player statistics |
| `playerArchives` | `/player/{username}/games/archives` | Get player game archives |
| `playerGames` | `/player/{username}/games/{year}/{month}/pgn` | Get monthly player games as PGN |
| `club` | `/club/{club}` | Get club information |
| `clubMembers` | `/club/{club}/members` | Get club members |
| `clubMatches` | `/club/{club}/matches` | Get club matches |
| `country` | `/country/{country}` | Get country information |
| `countryPlayers` | `/country/{country}/players` | Get players from a country |
| `countryClubs` | `/country/{country}/clubs` | Get clubs from a country |
| `titled` | `/titled` | Get titled players |
| `leaderboards` | `/leaderboards` | Get Chess.com leaderboards |
| `streamers` | `/streamers` | Get Chess.com streamers |
...
GET /chess?type=playerArchives&username=hikaru
GET /chess?type=playerGames&username=hikaru&year=2026&month=08
GET /chess?type=country&country=IN
GET /chess?type=countryPlayers&country=IN
GET /chess?type=countryClubs&country=IN
GET /chess?type=titled&title=GM
GET /chess?type=leaderboards
GET /chess?type=streamers
GET /chess?type=club&club=chess-com
GET /chess?type=clubMembers&club=chess-com
GET /chess?type=clubMatches&club=chess-com
```

The default type is `dailyPuzzle`. Operations requiring a username, club,
country, year/month, or resource ID validate the required parameters and
return `400` when they are missing.

The `playerGames` operation returns PGN text rather than a JSON resource.
The service preserves the upstream content type when returning the response.
...
curl "http://localhost:3000/deck-of-cards?operation=new"

## Deck of Cards - shuffled deck
curl "http://localhost:3000/deck-of-cards?operation=newShuffle&deckCount=1"

## Deck of Cards - draw cards
curl "http://localhost:3000/deck-of-cards?operation=draw&deckId=3p40paa87x90&count=2"

## Deck of Cards - shuffle existing deck
curl "http://localhost:3000/deck-of-cards?operation=shuffle&deckId=3p40paa87x90&remaining=true"

## Deck of Cards - add cards to pile
curl "http://localhost:3000/deck-of-cards?operation=pileAdd&deckId=3p40paa87x90&pileName=discard&cards=AS,2S"

---

# 30. Digimon API

The Digimon API provides access to Digimon data, attributes, fields, levels, types, and skills.
...
| Parameter | Required | Description |
| --------- | -------- | ----------- |
| `type` | No | Operation type; defaults to `digimon` |
| `value` | Conditional | Resource ID or name |
| `name` | No | Digimon/resource name filter |
| `attribute` | No | Digimon attribute filter |
| `level` | No | Digimon level filter |
| `xAntibody` | No | X-Antibody filter |
| `page` | No | Page number |
| `pageSize` | No | Results per page |

### Supported Types

Supported operation types are `digimon`, `attribute`, `field`, `level`, `type`, and `skill`.

| Type | Upstream path | Description |
| ---- | ------------- | ----------- |
| `digimon` | `/digimon` | List or get a Digimon |
| `attribute` | `/attribute` | List or get an attribute |
| `field` | `/field` | List or get a field |
| `level` | `/level` | List or get a level |
| `type` | `/type` | List or get a Digimon type |
| `skill` | `/skill` | List or get a skill |
...
GET /digimon?type=digimon&value=1
GET /digimon?type=digimon&name=Agumon
GET /digimon?type=digimon&attribute=Vaccine
GET /digimon?type=digimon&level=Rookie
GET /digimon?type=digimon&xAntibody=true
GET /digimon?type=digimon&page=2&pageSize=20
GET /digimon?type=attribute
GET /digimon?type=attribute&value=1
GET /digimon?type=field
GET /digimon?type=field&value=1
GET /digimon?type=level
GET /digimon?type=level&value=1
GET /digimon?type=type
GET /digimon?type=type&value=1
GET /digimon?type=skill
GET /digimon?type=skill&value=1
```

The default type is `digimon`. Operations using `value` append the value to the
corresponding upstream resource path. Additional query parameters are forwarded
to the Digimon API.
...
The Dragon Ball API provides access to Dragon Ball characters, planets, and transformations.
...
| Parameter | Required | Description |
| --------- | -------- | ----------- |
| `type` | No | Operation type; defaults to `characters` |
| `value` | Conditional | Character, planet, or transformation ID |
| `page` | No | Page number |
| `limit` | No | Results per page |
| `name` | No | Character or planet name filter |
| `gender` | No | Character gender filter |
| `race` | No | Character race filter |
| `affiliation` | No | Character affiliation filter |
| `isDestroyed` | No | Planet destruction status filter |

### Supported Types

Supported operation types are `characters`, `character`, `planets`, `planet`,
`transformations`, and `transformation`.

| Type | Upstream path | Description |
| ---- | ------------- | ----------- |
| `characters` | `/characters` | List characters |
| `character` | `/characters/{id}` | Get a character by ID |
| `planets` | `/planets` | List planets |
| `planet` | `/planets/{id}` | Get a planet by ID |
| `transformations` | `/transformations` | List transformations |
| `transformation` | `/transformations/{id}` | Get a transformation by ID |

### Character Examples

```http
GET /dragon-ball
GET /dragon-ball?type=characters
GET /dragon-ball?type=characters&page=2&limit=10
GET /dragon-ball?type=characters&name=Goku
GET /dragon-ball?type=characters&race=Saiyan
GET /dragon-ball?type=characters&affiliation=Z%20fighter
...
```http
GET /dragon-ball?type=planets
GET /dragon-ball?type=planets&page=2&limit=10
GET /dragon-ball?type=planets&name=Earth
...
GET /dragon-ball?type=transformations&page=2&limit=10
GET /dragon-ball?type=transformation&value=1
GET /dragon-ball?type=transformation&value=2
```

The default type is `characters`. Operations using `value` append the value to
the corresponding upstream resource path. Pagination and supported filters are
forwarded to the Dragon Ball API.
...
```bash
curl "http://localhost:3000/digimon?type=digimon&value=Agumon"
```

## Digimon pagination

```bash
curl "http://localhost:3000/digimon?type=digimon&page=2&pageSize=20"
```

## Digimon attributes

```bash
curl "http://localhost:3000/digimon?type=attribute"
```

## Dragon Ball characters

```bash
curl http://localhost:3000/dragon-ball
```

## Dragon Ball character by ID

```bash
curl "http://localhost:3000/dragon-ball?type=character&value=1"
```

## Dragon Ball character search

```bash
curl "http://localhost:3000/dragon-ball?type=characters&name=Goku"
```

## Dragon Ball planets

```bash
curl "http://localhost:3000/dragon-ball?type=planets"
```

## Dragon Ball planet by ID

```bash
curl "http://localhost:3000/dragon-ball?type=planet&value=1"
```

## Dragon Ball transformations

```bash
curl "http://localhost:3000/dragon-ball?type=transformations"
```

## Dragon Ball transformation by ID
...
## Random User API random user

```bash
curl http://localhost:3000/random-user
```

## Random User API multiple users

```bash
curl "http://localhost:3000/random-user?results=10"
```

## Random User API Indian users

```bash
curl "http://localhost:3000/random-user?results=10&nat=in"
```

## Random User API selected fields

```bash
curl "http://localhost:3000/random-user?results=10&inc=name,email,picture"
```

## Random User API seeded results

```bash
curl "http://localhost:3000/random-user?results=10&seed=foobar&page=2"
```

## Bored random activity

```bash
curl http://localhost:3000/bored
```

## Bored educational activity

```bash
curl "http://localhost:3000/bored?operation=filter&type=education"
```

## Bored activity for two participants

```bash
curl "http://localhost:3000/bored?operation=filter&participants=2"
```

## Bored social activity for two participants

```bash
curl "http://localhost:3000/bored?operation=filter&type=social&participants=2"
```

## Bored activity by key

```bash
curl "http://localhost:3000/bored?operation=activity&value=3943506"
```

## Chess.com daily puzzle

```bash
curl http://localhost:3000/chess
```

## Chess.com random puzzle

```bash
curl "http://localhost:3000/chess?type=randomPuzzle"
```

## Chess.com player

```bash
curl "http://localhost:3000/chess?type=player&username=hikaru"
```

## Chess.com player statistics

```bash
curl "http://localhost:3000/chess?type=playerStats&username=hikaru"
```

## Chess.com player game archives

```bash
curl "http://localhost:3000/chess?type=playerArchives&username=hikaru"
```

## Chess.com monthly player games

```bash
curl "http://localhost:3000/chess?type=playerGames&username=hikaru&year=2026&month=08"
```

## Chess.com country players

```bash
curl "http://localhost:3000/chess?type=countryPlayers&country=IN"
```

## Chess.com titled players

```bash
curl "http://localhost:3000/chess?type=titled&title=GM"
```

## Chess.com leaderboards

```bash
curl "http://localhost:3000/chess?type=leaderboards"
```

## Chess.com streamers

```bash
curl "http://localhost:3000/chess?type=streamers"
```
...
---
# 32. Open Trivia DB API

The Open Trivia DB API provides trivia questions, category metadata, category
question counts, global question counts, and session-token management.
...
| Parameter | Required | Description |
| --------- | -------- | ----------- |
| `operation` | No | Local operation; defaults to `questions` |
| `amount` | No | Number of questions to return (up to 50) |
| `category` | No | Trivia category ID |
| `difficulty` | No | `easy`, `medium`, or `hard` |
| `type` | No | Question format: `multiple` or `boolean` |
| `encode` | No | Response encoding option |
| `token` | Conditional | Session token for question requests or token reset |
| `command` | Conditional | Token command: `request` or `reset` |

The local `operation` parameter selects the wrapper operation. The upstream
question-format parameter remains `type`, allowing requests such as
`operation=questions&type=multiple`.

### Supported Operations

| Operation | Upstream path | Description |
| --------- | ------------- | ----------- |
| `questions` | `/api.php` | Retrieve trivia questions |
| `categories` | `/api_category.php` | List trivia categories |
| `categoryCount` | `/api_count.php` | Get question counts for a category |
| `globalCount` | `/api_count_global.php` | Get global question counts |
| `token` | `/api_token.php` | Request or reset a session token |

### Examples

```http
GET /open-trivia
GET /open-trivia?operation=questions&amount=10
GET /open-trivia?operation=questions&amount=10&category=18
GET /open-trivia?operation=questions&amount=10&difficulty=easy
GET /open-trivia?operation=questions&amount=10&type=multiple
GET /open-trivia?operation=questions&amount=10&type=boolean
GET /open-trivia?operation=questions&amount=10&category=18&difficulty=hard&type=multiple
GET /open-trivia?operation=questions&amount=10&encode=url3986
...
Open Trivia DB can return API-level errors in its JSON `response_code` even
when the HTTP response is `200`. The service should preserve the upstream
response and distinguish these API-level outcomes from HTTP failures. The API
also applies rate limits; avoid rapid repeated requests.

---

# 33. TVmaze API

The TVmaze API provides access to television shows, episodes, cast and crew,
people search, and broadcast schedules.
...
| Parameter | Required | Description |
| --------- | -------- | ----------- |
| `type` | No | Local operation; defaults to `searchShows` |
| `value` | Conditional | Show, episode, or resource ID; or search term for supported search operations |
| `q` | Conditional | Search query for show or people search |
| `thetvdb` | Conditional | TVDB ID for `lookupShow` |
| `imdb` | Conditional | IMDb ID for `lookupShow` |
| `tvrage` | Conditional | TVRage ID for `lookupShow` |
| `country` | No | Country code for schedule requests |
| `date` | No | Date in `YYYY-MM-DD` format for schedule requests |
| `page` | No | Page number for the show index |
| Other query parameters | No | Forwarded to TVmaze where applicable |

### Supported Types

| Type | Upstream path | Description |
| ---- | ------------- | ----------- |
| `searchShows` | `/search/shows` | Search shows |
| `singleSearch` | `/singlesearch/shows` | Find one show by search term |
| `lookupShow` | `/lookup/shows` | Find a show by external ID |
| `show` | `/shows/{id}` | Get show details |
| `showEpisodes` | `/shows/{id}/episodes` | Get a show's episodes |
| `showCast` | `/shows/{id}/cast` | Get a show's cast |
| `showCrew` | `/shows/{id}/crew` | Get a show's crew |
| `showSeasons` | `/shows/{id}/seasons` | Get a show's seasons |
| `episode` | `/episodes/{id}` | Get episode details |
| `searchPeople` | `/search/people` | Search people |
| `schedule` | `/schedule` | Get a broadcast schedule |
| `webSchedule` | `/schedule/web` | Get a web schedule |
| `shows` | `/shows` | Get a page of the show index |
...
GET /tv-maze?type=showCast&value=82
GET /tv-maze?type=showCrew&value=82
GET /tv-maze?type=showSeasons&value=82
GET /tv-maze?type=episode&value=1
GET /tv-maze?type=searchPeople&q=lauren
GET /tv-maze?type=schedule&country=US&date=2026-09-17
GET /tv-maze?type=webSchedule&date=2026-09-17
GET /tv-maze?type=shows&page=1
```

The default type is `searchShows`. Search operations require a query (`q` or
`value`). ID-based operations require `value`. `lookupShow` requires at least
one external identifier (`thetvdb`, `imdb`, or `tvrage`). Invalid operation
types or missing required values return `400`. Query parameters are forwarded
to TVmaze, and upstream errors and timeouts are returned in the service envelope.

---

# 34. Studio Ghibli API

The Studio Ghibli API provides access to films, people, locations, species,
and vehicles.
...
| Parameter | Required | Description |
| --------- | -------- | ----------- |
| `type` | No | Operation type; defaults to `films` |
| `value` | Conditional | Resource ID for an individual-resource operation |
| Other query parameters | No | Forwarded to the upstream API |

### Supported Types

| Type | Upstream path | Description |
| ---- | ------------- | ----------- |
| `films` | `/films` | Get all films |
| `film` | `/films/{id}` | Get a film by ID |
| `people` | `/people` | Get all people |
| `person` | `/people/{id}` | Get a person by ID |
| `locations` | `/locations` | Get all locations |
| `location` | `/locations/{id}` | Get a location by ID |
| `species` | `/species` | Get all species |
| `specie` | `/species/{id}` | Get a species by ID |
| `vehicles` | `/vehicles` | Get all vehicles |
| `vehicle` | `/vehicles/{id}` | Get a vehicle by ID |

### Examples

```http
GET /studio-ghibli
GET /studio-ghibli?type=films
...
GET /studio-ghibli?type=location&value=LOCATION_ID
GET /studio-ghibli?type=species
GET /studio-ghibli?type=specie&value=SPECIES_ID
GET /studio-ghibli?type=vehicles
GET /studio-ghibli?type=vehicle&value=VEHICLE_ID
```

The default type is `films`. Individual-resource operations require `value`,
which is substituted into the upstream `{id}` path parameter. Invalid operation
types or missing required values return `400`. Query parameters are forwarded
to the upstream API, and upstream errors and timeouts are returned in the
...
| `GET`  | `/random-user`             | Random User API lookup       |
| `GET`  | `/bored`                   | Bored activity lookup        |
| `GET`  | `/deck-of-cards`          | Deck of Cards lookup        |
| `GET`  | `/chess`                   | Chess.com public API lookup  |
